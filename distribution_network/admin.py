...
class NetworkParticipantAdmin(admin.ModelAdmin):
    ...
    def supplier_link(self, obj):
        ...
        if obj.supplier:
            url = reverse(
                "admin:distribution_network_networkparticipant_change",
    ...
    def clear_debt_to_suppliers(self, request, queryset):
        ...
        # Показать сообщение
        self.message_user(
            request, f"Задолженность обнулена у {updated} участников", messages.SUCCESS
        )

    # Задать понятное название для выпадающего мень действий
    clear_debt_to_suppliers.short_description = (
        "Обнулить задолженность у выбранных участников"
    )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "<int:object_id>/clear_debt/",
                self.admin_site.admin_view(self.clear_debt_to_supplier),
                name="clear_debt_to_supplier",
            ),
        ]
